...
[![Tests](https://img.shields.io/badge/Tests-33-green.svg)]()
[![Score](https://img.shields.io/badge/Code%20Score-88%2F100-brightgreen.svg)]()
...
- ✅ **数据库连接池** — DBUtils PooledDB（max 10, cached 5），不可用时优雅降级
- ✅ **统一 API 信封** — `{success, data, error, message}` 标准化响应
- ✅ **输入参数校验** — 9 个校验函数（分页/日期/排序/UUID/店铺名/平台名）
- ✅ **速率限制** — TokenBucket 令牌桶算法，按 IP 限流
- ✅ **统一日志** — 标准格式 + 文件轮转（50MB/30文件）
- ✅ **Docker 一键部署** — MySQL + Redis + App + Worker 四服务编排
- ✅ **RBAC 权限** — 角色-权限映射，页面级 + API 级控制
- ✅ **33 个单元/集成测试** — pytest + mock fixtures
...
└── src/
    ...
    ├── docker-compose.yml           # 一键编排（MySQL+Redis+App+Worker）
    ...
    │
    ├── rpa/                        # 📡 RPADataHub
    │   ├── blueprint.py            # 蓝图封装（路由已拆分至 routes/）
    │   ├── admin_server.py         # 独立 Admin (5100)
    │   ├── worker.py               # Worker 执行器（支持 --direct 模式）
    │   ├── file_watcher.py         # 文件监听
    │   ├── core/                   # 🆕 基础设施层（shared / logger / validators / rate_limit）
    │   ├── routes/                 # 🆕 拆分后路由（etl / monitor / collection / tasks / shops / bi / rbac）
    │   ├── static/js/              # 🆕 前端 JS 模块化（collection_health.js / shops.js）
...
| **速率限制** | TokenBucket 令牌桶算法，按 IP 限流 |
| **输入校验** | 9 个校验函数：分页/日期/排序/UUID/店铺名 |
| **Cookie 隔离** | `cookies/` `profiles/` `browser/` 全部 gitignore |
...
```bash
# 启动全部服务（MySQL + Redis + App + Worker）
docker-compose up -d

# 初始化数据库
docker-compose exec mysql mysql -uroot -p data < src/rpa/sql/init_tables.sql
...
| 版本 | 日期 | 变更 |
|:---|:---|:---|
| v1.3 | 2026-06-28 | 架构优化 v1：DBUtils连接池、统一API信封、输入校验、速率限制、统一日志、Docker部署、大文件拆分(1719→951行)、前端JS模块化、33个测试用例 |
