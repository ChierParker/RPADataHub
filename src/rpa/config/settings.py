...
class PathConfig:
    ...
    # 日志目录
    log_dir: str = _env("LOG_DIR", r"D:\EcomIQ-RPA\rpa_logs")
...
class WinAutoConfig:
    """Windows 桌面自动化配置"""
    # pywinauto 默认 backend: "uia" | "win32"
    default_backend: str = _env("WIN_DEFAULT_BACKEND", "uia")
    # 应用启动后等待时间（秒）
    app_startup_wait: float = 2.0
    # 默认操作超时（秒）
    default_timeout: int = 60
    # 图像识别模板目录
    template_dir: str = "templates/images"
    # OCR 引擎: "paddle" | "tesseract" | "auto"
    ocr_engine: str = _env("WIN_OCR_ENGINE", "auto")
    # OCR 默认置信度
    ocr_confidence: float = 0.7
    # 图像匹配默认置信度
    image_confidence: float = 0.8
    # 是否启用多尺度图像匹配
    image_multiscale: bool = False
    # 操作失败最大重试次数
    max_retries: int = 3
    # 截屏保存目录
    screenshot_dir: str = "data/screenshots"
    # 模板匹配日志目录
    flow_log_dir: str = "data/flow_logs"
...
class AppConfig:
    ...
    win_auto: WinAutoConfig = field(default_factory=WinAutoConfig)
